-- TetraVim Endpoints panel  (tetravim.util.endpoints_panel)
--
...
-- are merged:
--
--   * Spring MVC / WebFlux mappings  -- tetravim.util.spring.find_endpoints
--     (the VMware Spring Boot LS `workspace/symbol` model when attached, the
--     ripgrep + Tree-sitter scan otherwise).
--   * JSON OpenAPI / Swagger specs   -- tetravim.util.openapi.discover_specs +
--     list_endpoints.
--
...
---@return string root
---@return table|nil detected  the tetravim.util.spring.detect_root result
local function resolve_root()
  local ok, spring = pcall(require, "tetravim.util.spring")
  if ok then
...
--- Normalise a Spring endpoint (from tetravim.util.spring) into the shared row
--- item shape.
...
--- Normalise an OpenAPI operation (from tetravim.util.openapi.list_endpoints)
...
--- Async: tetravim.util.spring.find_endpoints resolves on vim.schedule.
function M.open()
  ...
  local ok_oa, openapi = pcall(require, "tetravim.util.openapi")
  if ok_oa then
  ...
  local ok_sp, spring = pcall(require, "tetravim.util.spring")
  if not ok_sp then
    render(0)
    return
  end
