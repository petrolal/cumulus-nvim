...
function M.run()
  ...
  local tool, root = require("tetravim.util.jvm.build").detect(cwd)
  ...
  root = root or cwd
  local sync_runner = require("tetravim.util.jvm.sync_runner")

  if tool == "maven" then
    local base_cmd = require("tetravim.util.jvm.build").wrapper_cmd("maven", root)
    sync_runner.run({
  ...
  elseif tool == "gradle" then
    local base_cmd = require("tetravim.util.jvm.build").wrapper_cmd("gradle", root)
    sync_runner.run({
