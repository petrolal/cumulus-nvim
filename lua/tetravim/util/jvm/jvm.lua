...
-- tool detection is native (tetravim.util.jvm.build); task/goal lists come from the
...
function M.setup_keymaps()
  ...
  -- in one place: tetravim.util.jvm.build.wrapper_cmd.
  ...
  map("n", "<leader>jta", function()
    ...
    -- Gradle/Maven runner (tetravim.util.jvm.test); anything mixed or Groovy
    ...
    if #roots > 0 and kinds.kotlin and not kinds.java and not kinds.scala and not kinds.other then
      require("tetravim.util.jvm.test").run_all()
      return
    end
  ...
  map("n", "<leader>jcl", function()
    require("tetravim.util.quality.coverage").load()
  ...
  map("n", "<leader>jcx", function()
    ...
    require("tetravim.util.quality.coverage").clear(false)
  end, { desc = "Clear Coverage Overlays" })

  map("n", "<leader>jct", function()
    require("tetravim.util.quality.coverage").toggle()
  end, { desc = "Toggle Coverage Display" })

  map("n", "<leader>jcs", function()
    require("tetravim.util.quality.coverage").summary()
  ...
  map("n", "<leader>jsq", function()
    require("tetravim.util.jvm.lsp_toggle").toggle()
  ...
  map("n", "<leader>jps", function()
    require("tetravim.util.quality.profiling").start()
  end, { desc = "Profiler: Start" })

  map("n", "<leader>jpx", function()
    require("tetravim.util.quality.profiling").stop()
  end, { desc = "Profiler: Stop" })

  map("n", "<leader>jpv", function()
    require("tetravim.util.quality.profiling").view()
  end, { desc = "Profiler: View Flamegraph" })

  map("n", "<leader>jpp", function()
    require("tetravim.util.quality.profiling").capture()
  ...
  map("n", "<leader>jns", function()
    require("tetravim.util.jvm.project_wizard").new_spring_boot()
  end, { desc = "New Spring Boot Project (Initializr)" })

  map("n", "<leader>jnm", function()
    require("tetravim.util.jvm.project_wizard").new_maven_project()
  end, { desc = "New Maven Project (Archetype)" })

  map("n", "<leader>jng", function()
    require("tetravim.util.jvm.project_wizard").new_gradle_project()
  ...
  -- (plugins/ui-whichkey.lua) via require("tetravim.util.jvm.jvm").whichkey_spec().
