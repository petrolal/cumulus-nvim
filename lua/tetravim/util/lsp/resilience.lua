...
function M.health()
  ...
  local async_ok, async = pcall(require, "tetravim.util.lsp.async")
  if async_ok and type(async.request_all_async) == "function" then
    h.ok("tetravim.util.lsp.async.request_all_async: available (non-blocking LSP fan-out)")
  else
    h.error("tetravim.util.lsp.async: failed to load or missing request_all_async")
  end
  ...
  -- through tetravim.util.lsp.async (and never a synchronous buf_request_all)
  -- is asserted structurally by lsp_resilience_spec.lua.
  local refactor_ok, refactor = pcall(require, "tetravim.util.edit.refactor")
  if refactor_ok and type(refactor.project_rename) == "function" then
    h.ok("Project-wide rename available (tetravim.util.edit.refactor.project_rename); async fan-out covered by specs")
  else
    h.warn("tetravim.util.edit.refactor failed to load or exposes no project_rename")
  end
