...
describe("Startup smoke (validate.sh stage 1)", function()
  it("verifies shell scripts syntax", function()
    local root = vim.fn.fnamemodify(debug.getinfo(1, 'S').source:sub(2), ':p:h:h:h:h')
    local scripts = { "bootstrap.sh", "scripts/headless-setup.sh", "scripts/validate.sh" }
...
describe("Startup smoke (validate.sh stage 5)", function()
  it("verifies LSP, completion & UI specs", function()
    local root = vim.fn.fnamemodify(debug.getinfo(1, 'S').source:sub(2), ':p:h:h:h:h')
    local code = "assert(pcall(require, 'cmp'), 'cmp not available'); assert(pcall(require, 'lspconfig'), 'lspconfig not available'); assert(pcall(require, 'render-markdown'), 'render-markdown not available'); assert(pcall(require, 'persistence'), 'persistence not available')"
...
describe("Startup smoke (validate.sh stage 5.1)", function()
  it("verifies File Explorer (oil.nvim) and keymaps", function()
    local root = vim.fn.fnamemodify(debug.getinfo(1, 'S').source:sub(2), ':p:h:h:h:h')
    local code = "assert(pcall(require, 'oil'), 'oil module not found'); local maps = vim.api.nvim_get_keymap('n'); local found = false; for _, m in ipairs(maps) do if m.lhs == '<Space>e' or m.lhs == ' e' or m.lhs == '<leader>e' then found = true; break end end; assert(found, '<leader>e keymap not found')"
    local out = vim.fn.system({ 'nvim', '--headless', '-u', root .. '/init.lua', '-c', 'lua ' .. code, '-c', 'qa!' })
